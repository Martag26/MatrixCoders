...
<head>
    ...
    <style>
        :root {
            ...
            --mc-dark: #1B2336; --mc-border: #e5e7eb;
            --mc-soft: #f8fafc; --mc-muted: #6b7280;
        }
        .chatbot-main { flex:1; min-width:0; padding:2rem 1.5rem; display:flex; flex-direction:column; }
        .cb-header { margin-bottom:1.25rem; display:flex; align-items:flex-start; justify-content:space-between; gap:12px; flex-wrap:wrap; }
        .cb-header-left {}
        .cb-title { font-size:1.4rem; font-weight:800; color:var(--mc-dark); margin:0 0 4px; display:flex; align-items:center; gap:.6rem; }
        ...
        .cb-restart-btn { display:inline-flex; align-items:center; gap:6px; padding:7px 14px; border:1.5px solid var(--mc-border); border-radius:8px; background:#fff; color:var(--mc-muted); font-size:.8rem; font-weight:600; cursor:pointer; transition:all .15s; white-space:nowrap; }
        .cb-restart-btn:hover { border-color:var(--mc-green); color:var(--mc-green); }
        .cb-wrap { flex:1; display:flex; flex-direction:column; max-width:800px; width:100%; background:#fff; border:1.5px solid var(--mc-border); border-radius:16px; overflow:hidden; box-shadow:0 4px 20px rgba(0,0,0,.05); }
        .cb-messages { flex:1; overflow-y:auto; padding:1.25rem; display:flex; flex-direction:column; gap:.85rem; min-height:400px; max-height:540px; scrollbar-width:thin; scrollbar-color:#e5e7eb transparent; }
        ...
        .cb-bubble { max-width:82%; padding:.7rem 1rem; border-radius:12px; font-size:.88rem; line-height:1.65; }
        ...
        .cb-suggest-btn { padding:.32rem .75rem; border:1.5px solid var(--mc-border); border-radius:99px; font-size:.78rem; font-weight:600; background:#fff; color:var(--mc-muted); cursor:pointer; transition:all .15s; }
        ...
        .cb-input { flex:1; border:1.5px solid var(--mc-border); border-radius:10px; padding:.55rem .9rem; font-size:.88rem; color:var(--mc-dark); resize:none; outline:none; transition:border-color .15s; min-height:42px; max-height:120px; overflow-y:auto; }
        ...
        .cb-divider { display:flex; align-items:center; gap:8px; color:var(--mc-muted); font-size:.72rem; font-weight:600; margin:4px 0; }
        .cb-divider::before,.cb-divider::after { content:''; flex:1; height:1px; background:var(--mc-border); }
    </style>
</head>
<body>
            ...
            <div class="chatbot-main">
                ...
                <div class="cb-header">
                    <div class="cb-header-left">
                        ...
                        <p class="cb-sub">Asistente virtual de soporte y orientación. Recuerda el contexto de tu conversación.</p>
                    </div>
                    <button class="cb-restart-btn" id="btnReiniciar" title="Nueva conversación">
                ...
                <div class="cb-wrap">
                    ...
                    <div class="cb-messages" id="cbMessages">
                        <div class="cb-msg">
                            ...
                            <div class="cb-bubble bot">
                                ...
                                Recuerdo el contexto de nuestra conversación. ¿En qué puedo ayudarte hoy?
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="cb-info">Oráculo usa Gemini IA · Los errores son posibles · Requiere GEMINI_API_KEY</div>
                </div>
...
function sugerirPregunta(btn) { inputEl.value = btn.textContent; inputEl.focus(); }
...
document.getElementById('btnReiniciar').addEventListener('click', async function () {
    this.disabled = true;
    try {
        const fd = new FormData(); fd.append('accion', 'reiniciar');
        await fetch(BASE_URL + '/index.php?url=chatbot', { method: 'POST', body: fd });
        messagesEl.innerHTML = '';
        const div = document.createElement('div'); div.className = 'cb-divider'; div.textContent = 'Nueva conversación';
        messagesEl.appendChild(div);
        addMsg('¡Hola de nuevo! Conversación reiniciada. ¿En qué puedo ayudarte?', 'bot');
    } finally {
        this.disabled = false;
    }
});
